...
class Order extends Base {
    ...
    public function order_list(){
       ...
       if($search_key)
       {
        ...
        $where11 = array('user_id'=>$this->user_id);
        $order = M('order')->where($where11)->select();
        $num = array('all'=>0,'unnpaid'=>0,'unresevied'=>0,'resevied'=>0,'unsent'=>0);
        foreach ($order as $key => $value) {
            if ($value['pay_status'] == 0 && $value['order_status'] == 0) {
                $num['unnpaid']++;
            }
            if ($value['pay_status']!= 0 && $value['shipping_status'] == 0) {
                $num['unsent']++;
            }
            if ($value['pay_status']!= 0 && $value['shipping_status'] !=0 && $value['confirm_time'] == 0) {
                $num['unresevied']++;
            }
            if ($value['pay_status']!= 0 && $value['shipping_status'] !=0 && $value['confirm_time'] != 0 &&$value['order_status'] == 2) {
                $num['resevied']++;
            }
            $num['all']++;
        }
    ...
    public function order_detail(){
        ...
        $this->assign('shipping_status',C('SHIPPING_STATUS'));
        $this->assign('pay_status',C('PAY_STATUS'));
        $this->assign('list',$order_info['goods_list']);
