...
class Content extends Base {
    ...
    public function index(){      
...

        {
            ...
            S('index_hot_goods',$index_hot_goods,JHSHOP_CACHE_TIME);
...

        {
        	...
        	S('index_recommend_goods',$index_recommend_goods,JHSHOP_CACHE_TIME);
        ...
        $hot_category = M('goods_category')->where("is_hot=1 and level=3 and is_show=1")->cache(true,JHSHOP_CACHE_TIME)->select();//热门三级分类
        ...
        $cateList['new'] = $GoodsLogic->getNewGoods();

        $cateList['hot'] = $GoodsLogic->getHotGood();

        $cateList['recommend'] = $GoodsLogic->getRecommendGoods();
        ...
        if(IS_POST){
 			echo '<script language="javascript">alert("我们会尽快和您取得联系!")</script>';
        }
    ...
    public function qr_code_raw(){        
        ...
        //http://www.jh-shop.cc/Home/Index/erweima/data/www.99soubao.com
    ...
    public function ajax_favorite(){
        ...
        $favourite_goods = Db::name('goods')->where($where)->order('goods_id DESC')->page($p,$i)->cache(true,JHSHOP_CACHE_TIME)->select();//首页推荐商品
	...
	public function mailer(){
		if(IS_POST){
			...
			$lianluo = Request::instance()->post('lianluo');
				
				$mail = new Mail();
				if($lianluo=='kefu'){
					if(!$mail->sendmail('service@pure-seven.com',$name.'联系我们',$content)){

						$this->success("我们会尽快给您答复！");
					...
					if(!$mail->sendmail('contact@pure-seven.com',$name.'联系我们',$content)){
