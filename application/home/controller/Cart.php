...
class Cart extends Base {
    ...
    public function index(){
		...
		foreach($cartList as $k1=>$v1){
			if(isset($v1['goods']) && !empty($v1['goods'])){
				if(isset($v1['goods']->flag_china)){
					$flag_china = $v1['goods']->flag_china;
					if($flag_china == 0){
						$shiming_is = 1;
					}
				}
        ...
        $shippingList = M('Plugin')->where("`type` = 'shipping' and status = 1")->cache(true,JHSHOP_CACHE_TIME)->select();// 物流公司
        ...
        $this->assign('address_num',count($address_list,0));
		$this->assign('shiming_is', $shiming_is);
        //dump($address_list);
        return $this->fetch();
    }
    ...
    public function cartis(){
        $goods_id = I('get.id/d');
        $goods_num = I('get.goods_num/d');
        $model = M('goods')->where('goods_id','eq',$goods_id)->select();
    ...
    public function cart2(){		
		...
		$shiming_is = 0;
		foreach($cartList as $k1=>$v1){
			if(isset($v1[0]['goods']) && !empty($v1[0]['goods'])){
				if(isset($v1[0]['goods']->flag_china)){
					$flag_china = $v1[0]['goods']->flag_china;
					if($flag_china == 0){
						$shiming_is = 1;
					}
				}
        ...
        $datas = input('');//echo "<pre>";print_r($_POST);die;
        if((int)$datas['groupid'] > 0){
            $groups = M('group_buy')->where(array('id'=>$datas['groupid']))->find();
            if(!$groups) $this->error('拼团不存在');
            $map['user_id'] = $this->user_id;
            $map['group_id'] = $datas['groupid'];
            $map['pay_status'] = 1;
            $isset = M('order')->where($map)->find();
           // if($isset) $this->error('已参加此拼团');
            $this->assign('group_id',$datas['groupid']);
			$this->assign('zhuituan',$_POST['zhuituan']);
            $this->assign('group_type',$groups['group_id']);

            if($groups['group_id'] == 2){
                $cartList['cartList'][0]["goods_fee"] = $cartList['cartList'][0]["goods"]["group_price2"];
            }elseif($groups['group_id'] == 3){
                $cartList['cartList'][0]["goods_fee"] = $cartList['cartList'][0]["goods"]["group_price3"];
            }
			
			$cartList['cartList'][0]["goods_num"] = 1;
			
        }
        $Mgoods = M('goods');
        if ($datas['pack']) {
            foreach ($datas['pack'] as $k => $v) {
                $_goods =  $Mgoods->where(array('goods_id'=>$v))->find();
                $_goods['goods'] = $_goods;
                $_goods['ispack'] = 1;
                $_goods['goods_num'] = 1;
                $_goods['goods_fee'] = $_goods['pack_price'];
                
                $cartList['cartList'][] = $_goods;
            }
        }
        //dump($shiming_is);
        ...
        $address_list = M('UserAddress')->where(['user_id'=>$this->user_id,'is_pickup'=>0])->select();//用户收货地址
        if($address_list){
            $area_id = array();
            foreach ($address_list as $val){
                $area_id[] = $val['province'];
    ...
    public function cart3(){
        ...
        if($action == 'buy_now'){
            ...
            $order_goods[0]['pack_goods_sn'] = time().rand(10000,99999);
            
            $tax_price = $result['result']['buy_goods']['goods']['tax_price'];
            
            if ($pack) {
                $is_pack = 1;
                foreach ($pack as $k => $v) {
                    
                    $cartLogic->setGoodsModel($v);
                    $cartLogic->setGoodsBuyNum(1);
                    $orther = $cartLogic->buyNow();

                    
                    $pack_prices += $orther['result']['buy_goods']['goods']['pack_price'];
                    $tax_price += $orther['result']['buy_goods']['goods']['tax_price'];
                    $orther['result']['buy_goods']['total_fee'] = $pack_prices;
                    $orther['result']['buy_goods']['goods_fee'] = $pack_prices;
                    $orther['result']['buy_goods']['member_goods_price'] = $pack_prices;
                    $orther['result']['buy_goods']['pack_sn'] = $order_goods[0]['pack_goods_sn'];
                    $order_goods[$v] = $orther['result']['buy_goods'];
        ...
        if((int)$datas['group_id'] > 0){
            $groups = M('group_buy')->where(array('id'=>$datas['group_id']))->find();
            if(!$groups) $this->error('拼团不存在');
            $map['user_id'] = $this->user_id;
            $map['group_id'] = $datas['group_id'];
            $map['pay_status'] = 1;
            $isset = M('order')->where($map)->find();
            if($isset) $this->error('已参加此拼团');
            ...
            if($groups['group_id'] == 2){
                /*$result['result']['order_amount'] = $result['result']['order_goods'][0]["goods"]["group_price2"];*/
				$result['result']['order_amount'] = $result['result']['order_goods'][0]["goods"]["group_price2"]+$result['result']['shipping_price'];
				$result['result']['goods_price'] = $result['result']['order_goods'][0]["goods"]["group_price2"];
			}elseif($groups['group_id'] == 3){
                /*$result['result']['order_amount'] = $result['result']['order_goods'][0]["goods"]["group_price3"];*/
				$result['result']['order_amount'] = $result['result']['order_goods'][0]["goods"]["group_price3"]+$result['result']['shipping_price'];
				$result['result']['goods_price'] = $result['result']['order_goods'][0]["goods"]["group_price3"];
			}
		...
		if($action == 'buy_now'){
			...
			$tax_price = $result['result']['tax_price'];
		}

        $_payables = number_format($result['result']['order_amount'], 2, '.', '');
        ...
        if ($_REQUEST['act'] == 'submit_order') {
			...
			$zhuituan = $_POST['zhuituan'];		
            $result = $orderLogic->addOrder($this->user_id,$address_id,$shipping_code,$invoice_title,$coupon_id,$car_price,$user_note,$pay_name,$is_pack,$map['group_id'],$zhuituan); // 添加订单
    ...
    public function integral(){
        ...
        $shippingList = Db::name('Plugin')->where("`type` = 'shipping' and status = 1")->cache(true,JHSHOP_CACHE_TIME)->select();// 物流公司
