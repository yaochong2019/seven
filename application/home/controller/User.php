...
class User extends Base{
    ...
    public function _initialize() {      
        ...
        if(session('?user'))
        {
        	...
        	$nologin = array(
                ...
                'forget_pwd', 'check_captcha', 'check_username', 'send_validate_code','bind_account','bind_guide','bind_reg','do_login_phone','get_pcode'
        	);
    ...
    public function reg(){
        ...
        if(IS_POST){
            ...
            }else{				$username = I('post.username','');				if(!check_email($username)){					$this->ajaxReturn(array('status'=>-1,'msg'=>'请输入正确的邮件地址'));				}			}
...
//            $this->verifyHandle('user_reg');
            ...
            if(check_mobile($username)){
                if($reg_sms_enable && $check_phone){   //是否开启注册验证码机制
                    //手机功能没关闭
            ...
            $data = $logic->reg($username,$password,$password2,0,$invite,$nickname,'',$usercode);
            if($data['status'] != 1){
    ...
    public function add_address(){
       /* header("Content-type:text/html;charset=utf-8");
        if(IS_POST){
            ...
            $logic = new UsersLogic();
            $data = $logic->add_address($this->user_id,0,$address);
            if($data['status'] != 1)
   ...
   public  function recharge(){
    ...
    public function editBasicMessage()
    {
        ...
        if (IS_POST){
			...
			$userInfo = M('users')->where('mobile ='.$mobile.' and user_id!='.$this->user_id)->find();
