...
class Cart extends MobileBase {
    ...
    public function index(){
        ...
        $hot_goods = M('Goods')->where('is_hot=1 and is_on_sale=1')->limit(20)->cache(true,JHSHOP_CACHE_TIME)->select();
    ...
    public function cart2(){
        ...
        if((int)$datas['groupid'] > 0){
            ...
            $this->assign('group_id',$datas['groupid']);			$this->assign('zhuituan',$_POST['zhuituan']);
	...
	public function upcart(){
		...
		$row = M('Cart')->where("id", $carid)->save(array('goods_num'=>$num));
		if($row){
			$return_arr = array('status'=>1,'msg'=>'成功'); // 返回结果状态
    ...
    public function cart3(){
        ...
        if ($_REQUEST['act'] == 'submit_order') {
            ...
            $orderLogic->setCartList($order_goods);			$zhuituan = $_POST['zhuituan'];	
    ...
    public function cart4(){
        ...
        if(strstr($_SERVER['HTTP_USER_AGENT'],'MicroMessenger') && 1==2){
            //微信浏览器
    ...
    public function integral(){
        ...
        $shippingList = Db('Plugin')->where("`type` = 'shipping' and status = 1")->cache(true,JHSHOP_CACHE_TIME)->select();// 物流公司
