    ...
    */
    public function _initialize()
    {
        ...
        $nologin = array(
            ...
            'forget_pwd', 'check_captcha', 'check_username', 'send_validate_code', 'express' , 'bind_guide', 'bind_account','login_phone','get_pcode','do_login_phone'
        );
...
//            header("Location: " . U('Mobile/User/index'));
    ...
    //手机快捷登陆
    public function do_login_phone()
    {
        $username = trim(I('post.username'));
        $code = trim(I('post.code'));

        $api = new \app\home\controller\Api;
        $cvc = $api->new_check_validate_code();
        ...
        $logic = new UsersLogic();
        $res = $logic->login_phone($username);
        if($res['status'] == 1){
            $res['url'] =  urldecode(I('post.referurl'));
            session('user',$res['result']);
            setcookie('user_id',$res['result']['user_id'],null,'/');
    ...
    public function get_pcode()
    {
        ...
        $phone =  trim(I('post.username'));
        $isset = M('users')->where(array('mobile'=>$phone))->find();
        $re = 1;
        if(!$isset) {
            $re = -1;
            echo $re;exit;
    ...
    public function reg()
    {
        ...
        if (IS_POST) {
            ...
            $password2 = I('post.password2','123456');			
			$foider = I('post.foid','');
			
			cookie('first_leader',$foider,60);
            ...
            */
            ...
            if($is_bind_account && session("third_oauth")){ //绑定第三方账号
                ...
                $data = $logic->reg($username, $password, $password2, 0, $invite ,$nickname , $head_pic,$usercode);
                //用户注册成功后, 绑定第三方账号
    ...
    public function userinfo()
    {
		...
		$hui =  M('users')->where('user_id', 32)->find();		
    ...
    */
    ...
    public function editBasicMessage()
    {
        ...
        if (IS_POST){
            ...
            $ids = M('users')->where(array('user_id'=>$this->user_id))->save($post);
            if($ids) echo 1;
            else echo 0;
            exit;
        }
    ...
    public function editBasicMessage2()
    {
        ...
        if (IS_POST){
            ...
            $post = I('post.');
            //$post['user_id'] = $this->user_id;
            $edit['my_address'] = $post['my_address'];
            $edit['usercode'] = $post['usercode'];

            $ids = M('users')->where(array('user_id'=>$this->user_id))->save($edit);
            if($ids === false) {
                $info = array('status'=>-1,'msg'=>'操作失败请重试');
            ...
            if($password){
                $res = $logic->password_new($this->user_id,$password);
                if($res['status'] == -1){
            ...
            if($paypwd){
                $res = $logic->paypwd_new($this->user_id,$paypwd);
                if($res['status'] == -1) {
                    echo json_encode($res);
                    exit;
                }
    ...
    public function fenxiao()
    {
        ...
        $mysons = $m->where(array('first_leader'=>$this->user_id))->select();
        

        if($mysons){
            foreach ($mysons as $key => $value) {
                
                $_son = $m->where(array('first_leader'=>$value['user_id']))->select();
                ...
                $mysons[$key]['xiaji'] = count($_son);

                $map['order_status'] = 2;
                $map['user_id'] = array('in',$_sonarr);
                $yeji = M('order')->where($map)->sum('order_amount');
                $mysons[$key]['yeji'] = $yeji;
                
            }
        }

        $this->assign('mysons',$mysons);
        return $this->fetch();
    }
}
