...
//        Session::start();
        ...
        if(I('foid')){
            $_SESSION['foid'] = I('foid');
            cookie('first_leader',I('foid'));
        }
    ...
    public function public_assign()
    {
       ...
       $tpshop_config = array();
       $tp_config = M('config')->cache(true,JHSHOP_CACHE_TIME)->select();       
       ...
       $brand_list = M('brand')->cache(true,JHSHOP_CACHE_TIME)->field('id,cat_id,logo,is_hot')->where("cat_id>0")->select();              
       $this->assign('brand_list', $brand_list);
       $this->assign('tpshop_config', $tpshop_config);
       /** 修复首次进入微商城不显示用户昵称问题 **/
       $this->user_id = $user_id = cookie('user_id');
        if($this->user_id){
            $logic = new UsersLogic();
            $this->user = $logic->get_info($this->user_id); //当前登录用户信息
            $this->user = $this->user['result'] ;//当前登录用户信息
