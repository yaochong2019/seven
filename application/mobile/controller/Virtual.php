    ...
    public function _initialize()
    {
        ...
        if (session('?user')) {
            ...
            $nologin = array(
                ...
                'forget_pwd', 'check_captcha', 'check_username', 'send_validate_code','do_login_phone',
            );
    ...
    public function check_virtual_goods(){
        ...
        if($goods['is_virtual'] == 1 && $goods['virtual_indate']>time() && $goods['store_count']>0){
            ...
            if(!empty($goods_spec) && $goods_spec !='undefined'){
                $specGoodsPriceList = M('SpecGoodsPrice')->where(array('goods_id'=>$goods_id))->cache(true,JHSHOP_CACHE_TIME)->getField("key,key_name,price,store_count,sku"); // 获取商品对应的规格价钱 库存 条码
