...
use app\admin\logic\LogisticsLogic;
use think\AjaxPage;
use think\Page;
use think\Db;
...
class Order extends Base {
    ...
    public function ajaxindex(){
        ...
        $show = $Page->show();
        //获取订单列表
        $orderList = $orderLogic->getOrderList($condition,$sort_order,$Page->firstRow,$Page->listRows);		/**xcjz*/		foreach($orderList as $k =>$v){			$flag_china = M('order_goods')->field('order_id')->where('order_id='.$v['order_id'].' and flag_china =1')->find();			if(!empty($flag_china)){				$orderList[$k]['flag_china'] = 1;			}		}		/**xcjz*/
...
exit("此功能~待开发！");
    ...
    public function deliveryHandle(){
        ...
        $res = $orderLogic->LogisticsBillPrintRequest($data);
        //$res = $orderLogic->LogisticsSyncBillInfoRequest($data);
		//$res = $orderLogic->deliveryHandle($data);

        if(isset($res['Header']['Result']) && $res['Header']['Result'] !='T'){
            
            exit ("<script> alert('".$res['Header']['ResultMsg']."');history.go(-1);</script>");
        }
        //dump($data);
        //exit;
		if($res){
    ...
    public function delivery_info(){
        ...
        $shipping_list = M('plugin')->where(array('status'=>1,'type'=>'shipping'))->select();
		$users = M('users')->where(array('user_id'=>$order['user_id']))->select();
/*xcjz*/
$flag_china = 1;
...
$this->assign('flag_china',$flag_china);
/*xcjz*/
        $this->assign('shipping_list',$shipping_list);
