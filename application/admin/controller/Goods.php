...
class Goods extends Base {
    ...
    public function countryList(){     
        $GoodsLogic = new GoodsLogic();
        $cat_list = $GoodsLogic->goods_cat_lists();
        $this->assign('cat_list',$cat_list);
        return $this->fetch();
    }
    ...
    public function addEditCountry(){
            ...
            if(IS_GET)
            {
                $goods_category_info = D('GoodsCountry')->where('id='.I('GET.id',0))->find();
                $level_cat = $GoodsLogic->find_parent_cats($goods_category_info['id']); // 获取分类默认选中的下拉框
                
                $cat_list = M('goods_country')->where("parent_id = 0")->select(); // 已经改成联动菜单                
                $this->assign('level_cat',$level_cat);                
                $this->assign('cat_list',$cat_list);                 
                $this->assign('goods_category_info',$goods_category_info);
                return $this->fetch('_country');
                exit;
            }
            $GoodsCategory = D('GoodsCountry'); //
            $type = I('id') > 0 ? 2 : 1; // 标识自动验证时的 场景 1 表示插入 2 表示更新                        
            //ajax提交验证
            if(I('is_ajax') == 1)
            {   
                // 数据验证
                $validate = \think\Loader::validate('GoodsCountry');
                if(!$validate->batch()->check(input('post.')))
                {                          
                    $error = $validate->getError();
                    $error_msg = array_values($error);
                    ...
                    if($type == 2){
                        ...
                        $children = M('goods_country')->where($children_where)->max('level');
                        if (I('parent_id_1')) {
                            $parent_level = M('goods_country')->where(array('id' => I('parent_id_1')))->getField('level', false);
                            if (($parent_level + $children) > 4) {
                                $return_arr = array(
                                    'status' => -1,
                                    'msg'   => $parent_level.'区域馆最多为二级'.$children,
                                    'data'  => '',
                                );
                                $this->ajaxReturn($return_arr);
                            }
                        ...
                        if (I('parent_id_2')) {
                            $parent_level = M('goods_country')->where(array('id' => I('parent_id_2')))->getField('level', false);
                            if (($parent_level + $children) > 4) {
                                $return_arr = array(
                                    'status' => -1,
                                    'msg'   => '区域馆最多为二级',
                    ...
                    //查找同级分类是否有重复分类
                    $par_id = ($GoodsCategory->parent_id > 0) ? $GoodsCategory->parent_id : 0;
                    $same_cate = M('GoodsCountry')->where(['parent_id'=>$par_id , 'name'=>$GoodsCategory['name']])->find();
                    if($same_cate){
                        $return_arr = array(
                            'status' => 0,
                            'msg' => '同级已有相同区域存在',
                    ...
                    {
                        $GoodsCategory->isUpdate(true)->save(); // 写入数据到数据库
                        $GoodsLogic->refresh_cats(I('id'));
                    }
                    else
                    {
                        $GoodsCategory->save(); // 写入数据到数据库
                        $insert_id = $GoodsCategory->getLastInsID();
                        $GoodsLogic->refresh_cats($insert_id);
                    }
                    $return_arr = array(
                        'status' => 1,
                        'msg'   => '操作成功',
                        'data'  => array('url'=>U('Admin/Goods/CountryList')),
                    );
                    $this->ajaxReturn($return_arr);
    ...
    public function delGoodsCountry(){
        $ids = I('post.ids','');
        empty($ids) &&  $this->ajaxReturn(['status' => -1,'msg' =>"非法操作！",'data'  =>'']);
        // 判断子分类
        $count = Db::name("goods_country")->where("parent_id = {$ids}")->count("id");
        $count > 0 && $this->ajaxReturn(['status' => -1,'msg' =>'该分类下还有分类不得删除!']);
        // 判断是否存在商品
        $goods_count = Db::name('Goods')->where("cat_id = {$ids}")->count('1');
        $goods_count > 0 && $this->ajaxReturn(['status' => -1,'msg' =>'该分类下有商品不得删除!']);
        // 删除分类
        DB::name('goods_country')->where('id',$ids)->delete();
        $this->ajaxReturn(['status' => 1,'msg' =>'操作成功','url'=>U('Admin/Goods/countryList')]);
    }
    ...
    public function addEditGoods()
    {
        ...
        if ((I('is_ajax') == 1) && IS_POST) {
            ...
            $Goods->cx_start_time = strtotime(I('cx_start_time'));
            $Goods->cx_end_time = strtotime(I('cx_end_time'));
        ...
        $country_list = M('goods_country')->where("parent_id = 0")->select();
        if(isset($goodsInfo['country_cat_id']) && !empty($goodsInfo['country_cat_id'])){
            $country_list2 = M('goods_country')->where("parent_id = ".$goodsInfo['country_cat_id'])->select();
            
            $this->assign('country_list2', $country_list2);
        }
        if ($goodsInfo['is_pack_price']==1) {
            $vo = explode('+',$goodsInfo['pack_id']);
            $where['goods_id']=array("in",$vo);
            $mole = M('Goods')->where($where)->select();
            foreach ($mole as $key => $val) {
                $vol .= $val['goods_name'].' / ￥'.$val['pack_price']."\n";
            }
            $goodsPack = rtrim($vol,'+');
        }
        $brandList = $GoodsLogic->getSortBrands();
        ...
        $this->assign('goodsPack',$goodsPack);//优惠套餐
        $goodsImages = M("GoodsImages")->where('goods_id =' . I('GET.id', 0))->select();
        $this->assign('goodsImages', $goodsImages);  // 商品相册
        //dump($goodsInfo['pack_id']);
    ...
    public function delPack(){
        $goods_id = I('POST.id');
        $pack = M("goods")->where("goods_id=".$goods_id)->setField(array('is_pack_price'=>0,'pack_id'=>0));
        if ($pack !==false) {
