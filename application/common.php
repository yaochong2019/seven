...
function getCatGrandson ($cat_id)
{
    ...
    // 把整张表找出来
    $GLOBALS['category_id_arr'] = M('GoodsCategory')->cache(true,JHSHOP_CACHE_TIME)->getField('id,parent_id');
    // 先把所有儿子找出来
    $son_id_arr = M('GoodsCategory')->where("parent_id", $cat_id)->cache(true,JHSHOP_CACHE_TIME)->getField('id',true);
...
function update_pay_status($order_sn,$ext=array())
{
    if(stripos($order_sn,'recharge') !== false){
        ...
        if ($order['order_prom_type'] == 4) {
            ...
            if(isset($ext['transaction_id'])) $updata['transaction_id'] = $ext['transaction_id'];						$orderInfo = M('order')->where("order_sn", $order_sn)->find();			$group_id = $orderInfo['group_id'];			if(!empty($group_id)){				M('group_buy')->where("id", $group_id)->save(array('is_pay' => 1));									if(!empty($orderInfo['zhuituan'])){					$id = $group_id;										$group = M('group_buy')->where(array('id'=>$id))->find();					if(!empty($group)){						if(!empty($group['user_1']) && !empty($group['user_2'])) {						 						}elseif(!empty($group['user_1']) && empty($group['user_2'])) {							$data['user_2'] = $orderInfo['user_id'];						}elseif(empty($group['user_1']) ) {							$data['user_1'] = $orderInfo['user_id'];						}						$data['user_num'] = $group['user_num']+1;						if($data['user_num'] >= $group['group_id']){							$data['is_end'] = 1; 						}						$ids = M('group_buy')->where(array('id'=>$id))->save($data);											}				}							}
...
function calculate_price($user_id = 0, $order_goods, $shipping_code = '', $shipping_price = 0, $province = 0, $city = 0, $district = 0, $pay_points = 0, $user_money = 0, $coupon_id = 0)
{
    ...
    $goods_arr = M('goods')->where("goods_id in(" . implode(',', $goods_id_arr) . ")")->cache(true,JHSHOP_CACHE_TIME)
    ...
    $order_amount = $goods_price + $shipping_price +$tax_price- $coupon_price; // 应付金额 = 商品价格 + 物流费 - 优惠券
    ...
    $total_amount = $goods_price + $shipping_price +$tax_price;  //订单总价
 ...
 * @return type
 */
function get_goods_country_tree(){
    $tree = $arr = $result = array();
    $cat_list = M('goods_country')->cache(true)->where("is_show = 1")->order('sort_order')->select();//所有分类
    if($cat_list){
        foreach ($cat_list as $val){
            if($val['level'] == 2){
                $arr[$val['parent_id']][] = $val;
...
//时间距离
function time_distance($stime,$timetype=1,$etime=null)
{
    
    if(!$etime) $etime = time();
    if($timetype == 1) $stime = strtotime($stime);

    $btime = $stime - $etime;
    
    if($btime < 0) return '1分钟';
    $year = floor($btime/(365*24*60*60));
    if($year >= 1) return $year.'年';
    $month = floor($btime/(30*24*60*60));
    if($month >= 1) return $month.'月';
    $day = floor($btime/(24*60*60));
    if($day >= 1) return $day.'天';
    $hour = floor($btime/(60*60));
    if($hour >= 1) return $hour.'小时';
    $minute = ceil($btime/(60));
    if($minute >= 1){
        return $minute.'分钟';
    }else{
        return '1分钟';
