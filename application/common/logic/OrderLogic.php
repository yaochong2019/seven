...
class OrderLogic
{
	 ...
	 * @return array
	 */
	public function addOrder($user_id,$address_id,$shipping_code,$invoice_title,$coupon_id = 0,$car_price=[],$user_note='',$pay_name='',$is_pack=0,$group_id=0,$zhuituan=0)
	{
		...
		$shipping = M('Plugin')->where("code", $shipping_code)->cache(true,JHSHOP_CACHE_TIME)->find();
		...
		$data = array(
				...
				//'order_amount'     =>($car_price['payables'] + $car_price['tax_price']),//'应付款金额',
				...
				'is_pack'		   =>$is_pack,//是否是套餐				
		);		$data['group_id'] = $group_id;		$data['zhuituan'] = $zhuituan;
		...
		{
			$goods = M('goods')->where("goods_id", $val['goods_id'])->cache(true,JHSHOP_CACHE_TIME)->find();
			...
			$data2['flag_china']         = $val['flag_china'];
