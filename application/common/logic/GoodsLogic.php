...
class GoodsLogic extends Model
{
    ...
    public function get_filter_brand($goods_id_arr, $filter_param, $action)
    {
        ...
        $list_brand = M('brand')->where($_map)->limit('30')->select();
    ...
    public function commentStatistics($goods_id)
    {
        ...
        $c7 = M('comment')->where($commentWhere)->where($where)->count();
        ...
        return array('c0' => $c0, 'c1' => $c1, 'c2' => $c2, 'c3' => $c3, 'c4'=>$c4,'rate1' => $rate1, 'rate2' => $rate2, 'rate3' => $rate3,'c7'=> $c7);
    }
    ...
    public function collect_goods($user_id, $goods_id)
    {
        ...
        $coll = M('GoodsCollect')->where("user_id",$user_id)->where("goods_id", $goods_id)->find();
        if ($coll) {
            M('GoodsCollect')->where("user_id",$user_id)->where("goods_id", $goods_id)->delete();
            return array('status' => 3, 'msg' => '取消收藏成功', 'result' => array());
        }else{
    ...
    public function getGoodsPriceByLadder($goods_num, $goods_price, $price_ladder)
    {
        ...
        for ($i = 0; $i < $price_ladder_count; $i++) {
            ...
            if($goods_num >= $price_ladder[$i]['amount'] && $goods_num < $price_ladder[$i+1]['amount']){
                return $goods_price-$price_ladder[$i]['price'];//@xcjz@return $price_ladder[$i]['price'];
            }
