...
class ActivityLogic extends Model
{
    ...
    public function addGroupInfo(){
        ...
        $groupAddData = array(
            'group_id' => $this->$group_id,//几人团id
            'goods_id' => $this->$goods_id,//商品id
            'user_num' => $this->$group_id,//限制人数
            'user_id' => $this->$user_id,//用户id
            'head_id' => $this->$user_id,//团长id
            'item_id' => $this->$group['item_id'],//商品规格
            'order_id' => $this->$order_id,//订单编号id--
            'title' => $this->$group['title'],//标题
            'start_time'=>$start_time,//开始时间
            'end_time' => $end_time,//结束时间
            'price' => $this->$group['price'],//拼团价格
            'goods_price' => $this->$group['goods_price'],//商品原价
            'goods_name' => $this->$group['goods_name'],//商品名称
            'goods_num' => $this->$group['goods_num'],
            'is_end' => 1,
        );
        $model = M('group_buy')->where('goods_id','eq',$goods_id || 'is_end','eq',0)->select();
        if ($model) {
            $groupResult = Db::name('group_buy')->insertGetId($groupAddData);
            if($groupResult !== false){
            return array('status' => 1, 'msg' => '拼团创建成功！', 'result' => '');
        }else{
            return array('status' => -1, 'msg' => '拼团创建失败！', 'result' => '');
        }
        }
    }

    public function addGroupInfo2($array){
        
       //判断是否已经创建过
       $isset = M('group_buy')->where(array('head_id'=>$array['user_id'],
       ...
       if($isset){
            return array('status' => -1, 'msg' => '您已创建拼此商品的团', 'result' => '');
       }
       $array['head_id'] = $array['user_id'];
       $array['start_time'] = time();
       $array['end_time'] = $array['start_time'] + 24*60*60;
       $array['user_num'] = 1;//目前仅自己参团
       $ids = M('group_buy')->add($array);
       if($ids){
            return array('status' => 1, 'msg' => '拼团创建成功！', 'result' => $ids);
       }else{
            return array('status' => -1, 'msg' => '拼团创建失败！', 'result' => '');
       }
