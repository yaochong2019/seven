...
class UsersLogic extends Model
{
    ...
    public function login_phone($username)
    {
        if(!$username)
            $result= array('status'=>0,'msg'=>'请填写账号或密码');
     ...
     * @return array
     */
    public function reg($username,$password,$password2,$push_id = 0,$invite=array(),$nickname="",$head_pic="",$usercode=""){
    	$is_validated = 0 ;
    ...
    public function get_goods_collect($user_id){
        ...
        //获取我的收藏列表
            ...
            ->field('c.collect_id,c.add_time,g.goods_id,g.goods_name,g.goods_sn,g.market_price,g.shop_price,g.is_on_sale,g.store_count,g.cat_id,g.is_virtual')
            ->join('goods g','g.goods_id = c.goods_id','INNER')
    ...
    public function password_new($user_id,$new_password){
        $user = M('users')->where('user_id', $user_id)->find();
        if(strlen($new_password) < 6)
            return array('status'=>-1,'msg'=>'密码不能低于6位字符','result'=>'');
        ...
        $row = M('users')->where("user_id", $user_id)->save(array('password'=>encrypt($new_password)));
        if(!$row)
            return array('status'=>-1,'msg'=>'原登陆密码与新密码重复','result'=>'');
        return array('status'=>1,'msg'=>'修改成功','result'=>'');
    }
    ...
    public function paypwd_new($user_id,$new_password){
        if(strlen($new_password) < 6)
            return array('status'=>-1,'msg'=>'密码不能低于6位字符','result'=>'');

        $row = M('users')->where("user_id",$user_id)->update(array('paypwd'=>encrypt($new_password)));
        if(!$row){
            return array('status'=>-1,'msg'=>'原支付密码与新密码重复','result'=>'');
        }
        $url = session('payPriorUrl') ? session('payPriorUrl'): U('User/userinfo');
        session('payPriorUrl',null);
        return array('status'=>1,'msg'=>'修改成功','url'=>$url);
