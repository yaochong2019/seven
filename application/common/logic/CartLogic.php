...
class CartLogic extends Model
{
    ...
    public function buyNow(){
        ...
        $buyGoods = [
			...
			'flag_china'=>$this->goods['flag_china'],
    ...
    {   if (!$this->user_id) {
        ...
        if($this->goods['exchange_integral'] > 0){
            return ['status'=>0,'msg'=>'此为积分兑换类商品!','result'=>['url'=>U('Goods/goodsInfo',['id'=>$this->goods['goods_id'],'item_id'=>$this->specGoodsPrice['item_id']],'',true)]];
        }
        ...
        if(empty($this->specGoodsPrice) && !empty($specGoodsPriceCount)){
            return array('status' => -1, 'msg' => '此商品需选择规格参数!', 'result' => '');
        }
        //有商品规格，和没有商品规格
    ...
    private function addNormalCart(){
        ...
        if ($userCartGoods) {
            ...
            $cartAddData = array(
                ...
                'prom_type' => 0,   // 0 普通订单,1 限时抢购, 2 团购 , 3 促销优惠
                'prom_id' => 0,   // 活动id
                'country_id'=>$this->goods['country_cat_id_2'],
            );
            if($this->specGoodsPrice){
    ...
    public function getCartList($selected = 0){
        ...
        $cartList = $cart->with('promGoods,goods,goodsCountry')->where($cartWhere)->select();  // 获取购物车商品
...
//        $cartCheckAfterList = $cartList;
    ...
    public function AsyncUpdateCart($cart = [])
    {
        ...
        foreach($cartList as $cartKey=>$cartVal){			$cartList[$cartKey]['tax_price'] = M('goods')->where('goods_id='.$cartVal['goods_id'])->getfield('tax_price');
    ...
    public function getCartPriceInfo($cartList = null){
        ...
        if($cartList){
            foreach ($cartList as $cartKey => $cartItem) {	
				if(isset($cartItem['tax_price'])){
					$tax_price += $cartItem['tax_price'];
					$total_fee += $cartItem['goods_fee']+$cartItem['tax_price'];
				}else{
